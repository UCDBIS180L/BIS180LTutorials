...
grade_result(
  pass_if(~identical(.result,  {bnames10 %>% str_detect("A")}))
)
```
...
grade_result(
  pass_if(~identical(.result,  {bnames10 %>% str_subset("th")}))
)
```
...
grade_result(
  pass_if(~identical(.result,  {bnamesearly %>% str_subset("ez{1,2}")}))
)
```
...
grade_result(
  pass_if(~identical(.result, {bnames100 %>% str_subset("[AEIOU]")}))
)
```
...
```{r brackets2-check}
grade_result(
  pass_if(~identical(.result,  {letters %>% str_subset("[^aeiou]")}))
)
```
...
```{r bp1-check}
grade_result(
  pass_if(~identical(.result,  {fruit %>% str_subset("(berry|pepper)")}))
)
```
...
grade_result(
  pass_if(~identical(.result,  {fruit %>% str_subset("^[aeiou]")}))
)
```
...
grade_result(
  pass_if(~identical(.result, {fruit %>% str_subset("^[aeiou].*[aeiou]$")}))
)
```
...
grade_result(
  pass_if(~identical(.result,  {bnames10 %>% str_remove("[aeiou]")}))
)
```
...
grade_result(
  pass_if(~identical(.result,  {bnames10 %>% str_remove_all("[aeiou]")}))
)
```
...
grade_result(
  pass_if(~identical(.result,  {knames %>% str_replace("K", "C")}))
)
```
## Back references
...
grade_result(
  pass_if(~identical(.result,  {bnames100 %>% tolower() %>% str_subset("^(.).*\\1$")})),
  fail_if(~identical(.result, {bnames100 %>% tolower() %>% str_subset("(.).*\\1")}), "Nice try but maybe you forgot to specify the start and end of the string")
  )
```
...
grade_result(
  pass_if(~identical(.result,  {people %>% mutate(last_name=str_remove(full_name, ", .*"))}))
  )
```
...
grade_result(
  pass_if(~identical(.result,  { people %>% mutate(first_last=str_replace(full_name, "(.*), (.*)", "\\2 \\1"))}))
  )
```
